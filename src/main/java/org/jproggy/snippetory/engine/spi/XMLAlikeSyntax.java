...
public class XMLAlikeSyntax extends RegExSyntax {
	...
	public RegexParser parse(CharSequence data) {
		...
		Pattern start = Pattern.compile("\\<t:(" + NAME + ATTRIBUTES + ")[ \\t]*\\>");
		patterns.put(start, TokenType.BlockStart);

		start = Pattern.compile(LINE_START + "\\<t\\:(" + NAME + ATTRIBUTES + ")[ \\t]*\\>" + LINE_END, Pattern.MULTILINE);
		patterns.put(start, TokenType.BlockStart);

		Pattern end = Pattern.compile("</t\\:(" + NAME + ")\\>");
		patterns.put(end, TokenType.BlockEnd);

		end = Pattern.compile(LINE_START + "</t\\:(" + NAME + ")\\>" + LINE_END, Pattern.MULTILINE);
		patterns.put(end, TokenType.BlockEnd);
